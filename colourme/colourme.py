...
class ColourMe(commands.Cog):
    ...
    def _could_be_colourme(self, role):
    ...
    def _already_has_colourme(self, ctx, rolename):
        guild = ctx.message.guild
        return discord.utils.get(guild.roles, name=rolename)

    @commands.guild_only()
    @commands.group(name="colourme")
    async def colourme(self, ctx):
        """Change the colour of your name via custom roles."""
        pass

    @colourme.command(name="change")
    @commands.cooldown(10, 60, commands.BucketType.user)
    async def _change_colourme(self, ctx: commands.Context, newcolour: discord.Colour):
        """Change the colour of your name.

        New colour must be a valid hexidecimal colour value.
        """
        guild = ctx.message.guild
        ...
        for role in member.roles:
            if role.id in protected_roles:
                return await ctx.send("You have a role that is protected from colour changes.")
            if self._could_be_colourme(role):
    ...
    @colourme.command(name="clean")
    @checks.admin_or_permissions(manage_guild=True)
    async def _clean_colourme(self, ctx: commands.Context):
        """Clean colourme roles by removing all permissions."""
    ...
    @colourme.command(name="protect")
    @checks.admin_or_permissions(manage_guild=True)
    async def _protect_colourme(self, ctx, role: str):
        """Add a role to the list of protected roles.

        Members with this role as top role will not be allowed to change colour.
        Example: [p]colourme protect admin
        """
        guild = ctx.message.guild
        ...
        if protect_role.id in protected_roles:
            ...
            await ctx.send(f"Users with top role '{role}' are protected from colour changes.")

    @colourme.command(name="unprotect")
    @checks.admin_or_permissions(manage_guild=True)
    async def _unprotect_colourme(self, ctx, role: str):
        """Remove a role from the list of protected roles.

        Example: [p]colourme unprotect admin
        """
        guild = ctx.message.guild
        protect_role = discord.utils.get(guild.roles, name=role)
        if protect_role is None:
        ...
        if protect_role.id not in protected_roles:
            ...
            await self.conf.guild(guild).protected_roles.set(protected_roles)
            await ctx.send(f"Users with top role '{role}' are no longer protected from colour changes.")

    @colourme.command(name="listprotect")
    async def _listprotect_colourme(self, ctx):
        """Lists roles that are protected from colour changes."""
        guild = ctx.message.guild
        protected_roles = await self.conf.guild(guild).protected_roles()
